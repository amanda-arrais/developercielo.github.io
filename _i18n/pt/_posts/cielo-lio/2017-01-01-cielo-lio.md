...
## Cielo LIO - Versão target da aplicação
...
Para solicitar um terminal, acesse o Portal de Desenvolvedores > Suporte > Cielo Smart LIO > Envie sua pergunta > **para solicitar LIO** escolha a opção > Solicitação LIO DEV Teste / Credenciamento EC Cielo ou **para solicitar Cielo Smart** escolha a Solicitação Cielo Smart Teste/ Credenciamento EC Cielo. Nessa mesma solicitação, será realizado o credenciamento EC Cielo. Esse credenciamento é necessário para: a solicitação de terminal; a publicação e distribuição do aplicativo em produção; e o recebimento de pagamentos referente aos aplicativos publicados em loja pública.

### Configuração do DX8000 – Máquina de DEBUG 

Para confirmar seu DX8000 basta seguir o passo a passo abaixo:

**Pré requisitos iniciais:**

-	Certifique-se de que o terminal possui essa mensagem na tela: **"Cuidado: Modo de desenvolvimento. Transação de pagamento proibida."** Caso não esteja aparecendo essa mensagem entre em contato com o nosso suporte - [clique aqui](https://devcielo.zendesk.com/hc/pt-br/requests/new?ticket_form_id=360000614392)
-	Certifique-se de que o terminal possui **carga igual ou superior a 50%.** Isso garante que todo o procedimento ocorra sem interrupções e permite que atualizações sejam realizadas.
-	Certifique-se de que o terminal possui internet estável

**Atenção:   As transações realizadas no Modo de Desenvolvimento não impactam na agenda financeira do EC/NL cadastrado no terminal. As transações utilizam números de cartões virtuais, disponibilizados neste manual.**

Passo a passo:

-	**Abra um chamado no nosso Suporte [clicando aqui](https://devcielo.zendesk.com/hc/pt-br/requests/new?ticket_form_id=360000614392) para que possamos informar as demais informações como CNPJ e número lógico.**
- Ligue o Terminal e clique em **Vamos lá**.
-	Configure o Wi-fi para continuar.
-	Na tela de inserção de CPF ou CNPJ, clique no menu **"três pontinhos"** no topo da tela.

![TresPontinhos](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/CieloSmartOnboarding.jpg)

-	Selecione a opção **Acessar Funções Técnicas** e depois clique em **Prosseguir**.
-	Digite o número da função **69** e clique em **Executar função**.
-	Para inserir a senha ambiente, no Dev Console, acesse *Meus aplicativos*, clique em *Apps privados*, depois em *“Ver detalhes”* do aplicativo desejado e clique em *“Desinstalar app”* para visualizar a senha.
-	Selecione opção **Certificação** para a configuração do ambiente.
-	Retorne para a tela de inserção de CNPJ/CPF
-	Após entrar em contato com o Suporte aos Integradores insira o CNPJ que será informado pelo time Cielo. 
-	Confirme o cadastro para o processo de inicialização começar.
-	Após a finalização o terminal estará configurado e pronto para uso.
...
**Critérios para aprovação da certificação de aplicativo público:**
 ...
 - Manter seu app sempre nas versões mais atuais do SDK. Confira as versões atuais através do [Cielo Store Dev Console.](https://www.cieloliostore.com.br/sdk-cielo)

![DevConsole SDK](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/sdk_cielo_versao.png)
...
![cobranca1](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/cielo2.png)
...
Primeiro baixe e instale o aplicativo, download [aqui no link](https://s3-sa-east-1.amazonaws.com/cielo-lio-store/apps/lio-emulator/1.61.8/lio-emulator.apk)
...
![emulador1](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_1.png)

Depois de instalado, inicie o aplicativo, será aberta a tela de boas-vindas, nesta tela temos uma mensagem e um botão para limpar os dados (esta opção limpa os pedidos criados pelo integrador) e também possui um botão de configuração, que apresenta opções para as respostas automáticas em testes das chamadas de impressão. **Se achar necessário, pode minimizar o aplicativo, esta ação não muda sua experiência.**

![emulador2](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_2.png)

O próximo passo é instalar o aplicativo de integração, para fins didático sutilizamos nosso [aplicativo Sample](https://github.com/DeveloperCielo/LIO-SDK-Sample-Integracao-Local), disponibilizado no github da Cielo. Na imagem abaixo temos o seguinte cenário: Pedido criado, item adicionado, pedido liberado para pagamento (os passos descritos utilizam da integração com emulador).
...
![emulador4](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_4.png)

- O 1º campo é referente ao valor enviado no campo **“amount”*** da requisição de pagamento.
- O 2º campo é um dropdown menu com todos os Primary Products disponíveis para transação.
- O 3º campo é um dropdown menu com todos os Secondary Products disponíveis vinculados ao Primary Product selecionado.
- O 4º Campo são a lista de ECs disponiveis.

![emulador5](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_5.png)

Caso as opções selecionadas no dropdown do Primary Product seja “Crédito” e no Secondary Product alguma das opções de “Parcelamento”, um novo campo será exibido, mostrando a quantidade de parcelas a serem feitas naquela transação. Como ilustrado na imagem abaixo:

![emulador6](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_6.png)

Caso na requisição seja enviado os campos “Primary Product” e “Secondary Product”, os dropdown ficarão desabilitados para mudanças aqui na tela do emulador. Como ilustrado na imagem abaixo:

![emulador7](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_7.png)

**Sucesso:** Retorna os dados do pagamento simulando uma operação realizada com sucesso.
...
É possível testar a requisição de um cancelamento de pagamento. No cenário abaixo temos o cenário de cancelar um pagamento especifico que foi feito para um pedido.

![emulador8](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_8.png)

Ao clicar em Cancelar Pagamento é apresentado uma tela com as informações do pagamento.

![emulador9](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_9.png)

Após isso basta clicar no confirmar cancelamento (em um cenário de produção será encaminhado para um fluxo onde é preciso preencher alguns campos para que o cancelamento seja feito, no emulador como queremos testar apenas as chamadas que são feitas, para agilizar o prosseguimento do desenvolvimento, essas partes não será necessário). Basta escolher o tipo de resposta desejado após realizar uma chamada de cancelamento.

![emulador10](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_10.png)

## Impressão

Na imagem abaixo temos o seguinte cenário: No aplicativo sample temos algumas chamadas para realizar a impressão.

![emulador11](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_11.png)

Ao clicar em algum botão para imprimir, o emulador vem para foreground e apresenta a tela na qual é possível escolher qual cenário de resposta é desejado no teste.

![emulador12](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_12.png)

Caso deseja sempre obter a mesma resposta para todas as outras chamadas que forem feitas, basta selecionar a opção de Não perguntar novamente.

![emulador13](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_13.png)

Com a opção de não perguntar novamente marcada, as próximas respostas serão a que foi escolhida por último. Caso queira alterar o cenário e configurar uma outra resposta automática, é necessário voltar a tela de boas vindas, e clicar no botão de configurações.

![emulador14](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_14.png)

Nessa tela é possível habilitar para perguntar sempre, e voltar ao cenário padrão, onde em todas as chamadas de impressão será apresentado a tela com as opções, ou é possível escolher uma opção de resposta automática.

![emulador15](https://desenvolvedores.cielo.com.br/api-portal/sites/default/files/Emulador_15.png)
